...
class Music(commands.Cog, discordSuperUtils.CogManager.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.client_id = "9efa2636c34c421c8f0fb60281429094"
        self.client_secret = "f802ce2820a74789ae65f05494c66b38"
    ...
    async def on_music_error(self, ctx, error):
        ...
        embed = discord.Embed(title="Error!", color=ctx.author.colour)
    ...
    async def on_play(self, ctx, player):
        embed = discord.Embed(title="Playing Song...", color=ctx.author.colour)
    ...
    async def np(self, ctx):
        if player := await self.MusicManager.now_playing(ctx):
            ...
            embed = discord.Embed(
                title="Currently Playing", color=ctx.author.colour)
    ...
    async def volume_error(self, ctx, error):
        errors = {
            commands.BadArgument: 'please enter number',
        }
        embed = discord.Embed(title="Error!", color=ctx.author.colour)
    ...
    async def ls(self, ctx):
        if queue := await self.MusicManager.get_queue(ctx):
            ...
            if loop_status:
                embed = discord.Embed(
                    title=loop_status, color=ctx.author.colour)
